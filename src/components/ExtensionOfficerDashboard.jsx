...
function trustLabel(score) {
  if (score >= 75) return { label: 'Good', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' };
  if (score >= 55) return { label: 'Fair', color: 'text-amber-700 bg-amber-50 border-amber-200' };
  return { label: 'Needs Verification', color: 'text-rose-700 bg-rose-50 border-rose-200' };
}
...
export const ExtensionOfficerDashboard = ({ currentLang, onNavigate, onRoleChange }) => {
  ...
  const tBadge = trustLabel(farmerTrustScore);
        ...
        <div className="bg-[#0F382A] rounded-3xl p-6 sm:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="bg-[#0A261D] rounded-2xl p-4 border border-emerald-800 shrink-0 text-xs space-y-2 min-w-[220px]">
            ...
            <div className="border-t border-emerald-800 pt-2 flex justify-between space-x-4">
              <span className="text-emerald-300">Farmer Credibility:</span>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-5 hover:shadow-md transition-shadow">
              ...
              <div className="pb-3 border-b border-slate-100 space-y-3">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-xs">
                  {[
                    { id: 'all', label: `All (${sortedCases.length})`, cls: 'bg-[#0F382A] text-white', inactiveCls: 'bg-slate-100 text-slate-600 hover:bg-slate-200' },
                    { id: 'critical', label: `Critical (${criticalCount})`, cls: 'bg-rose-600 text-white', inactiveCls: 'bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200' },
                    { id: 'high', label: `High (${highCount})`, cls: 'bg-orange-500 text-white', inactiveCls: 'bg-orange-50 text-orange-700 hover:bg-orange-100 border border-orange-200' },
                    { id: 'pending', label: `Pending (${pendingCount})`, cls: 'bg-amber-600 text-white', inactiveCls: 'bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-200' },
                    { id: 'live', label: `Live AI (${liveCount})`, cls: 'bg-emerald-700 text-white', inactiveCls: 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200' },
                  ].map(tab => (
                    <button
                      key={tab.id}
                        ...
                        <div className="rounded-xl bg-slate-50 p-3 border border-slate-200/70 text-xs">
                          ...
                          >
                            <span className="flex items-center gap-1.5">
                              ...
                              <span>Why Urgent — Visit Priority Score: {c.visitScore} / 100</span>
                            </span>
                      ...
                      {!c.verified ? (
                        <div className="flex flex-wrap items-center gap-2 pt-1">
                          ...
                          >
                            ...
                            <span>Confirm Ground-Truth (+5 Trust)</span>
                          </button>
                          ...
                          >
                            ...
                            <span>False Positive (−3 Trust)</span>
                          </button>
